...
public class ProcessoController {
    ...
    public String dashboard(Model model, HttpSession session) {
        ...
        long qtdAguardandoAnalise = processoService.contarPorEstado(usuario, StatusProcesso.AGUARDANDO_ANALISE);
        long qtdEmTransito = processoService.contarPorEstado(usuario, StatusProcesso.AGUARDANDO_RECEBIMENTO_EXECUCAO);
        long qtdPagamento = processoService.contarPorEstado(usuario, StatusProcesso.AGUARDANDO_SOLICITACAO_PAGAMENTO);
        ...
        model.addAttribute("processos", processoService.listarRecentes(usuario));
    ...
    public String listarTodosProcessos(@RequestParam(required = false) String busca,
        ...
        model.addAttribute("processos", processoService.listarComFiltros(
                usuarioLogado, busca, status, tipoProcesso, setorId
        ));
    ...
    public String detalharProcesso(@PathVariable Long id, Model model, HttpSession session) {
        ...
        model.addAttribute("ultimaObservacaoInconforme",
                historicoRepository.findByProcessoId(id).stream()
                        .filter(h -> h.getObservacaoInconforme() != null && !h.getObservacaoInconforme().isBlank())
                        .max(Comparator.comparing(HistoricoProcesso::getDataTransicao))
                        .map(HistoricoProcesso::getObservacaoInconforme)
                        .orElse(null)
        );
    ...
    private Double converterValorFiltro(String valor) {
    ...
    public String validarMaterial(
        ...
        if (status == StatusValidacaoRecebimento.NAO_CONFORME && motivo != null && !motivo.isBlank()) {
    ...
    public String registrarInconformidade(
        ...
        boolean registrada = processoService.registrarInconformidade(processo, usuario, motivo);

        redirectAttributes.addFlashAttribute(
                registrada ? "mensagemSucesso" : "mensagemErro",
                registrada
                        ? "Não conformidade registrada com sucesso."
                        : "Esta etapa já possui uma não conformidade registrada."
        );
